...
describe('Order entity', () => {
    ...
    it('should get confirm display info correctly', () => {
        ...
        let seats = [seat, seat2, seat3, seat4];
        order.setSeats(seats);
        expect(order.getOrderDetailInfos().ticketList[0].seat).to.equal('A1');
        expect(order.getOrderDetailInfos().ticketList[1].seat).to.equal('A2');
        expect(order.getOrderDetailInfos().ticketList[2].seat).to.equal('A3');
        expect(order.getOrderDetailInfos().ticketList[3].seat).to.equal('A4');
        expect(order.getOrderDetailInfos().ticketList[0].ticketCategory).to.equal('Adult');
        expect(order.getOrderDetailInfos().ticketList[1].ticketCategory).to.equal('Adult');
        expect(order.getOrderDetailInfos().ticketList[2].ticketCategory).to.equal('Adult');
        expect(order.getOrderDetailInfos().ticketList[3].ticketCategory).to.equal('Child');
    });
});