...
class MovieInTheaterSpider(scrapy.Spider):
    name = "MovieInTheaterSpider"
    movieInTheaterInfoList = []
    
    def start_requests(self):
        urlForMoviesInTheater = "https://www.imdb.com/movies-in-theaters/?ref_=nv_mv_inth"
        yield scrapy.Request(url=urlForMoviesInTheater, callback=self.parseForMoviesInTheater)


    def parseForMoviesInTheater(self, response):
        movieInTheaterURLs = []
        for entry in response.xpath('//td[@class="overview-top"]/h4/a/@href').extract():
            yield scrapy.Request(url= "https://www.imdb.com" + entry, callback=self.parseInfoForMovieInTheater)
             


    def parseInfoForMovieInTheater(self, response):
    ...
    def parseTrailerURL(self, response):
        ...
        if len(movieInfo['trailers']) == trailerCount:
            if not movieInfo in self.movieInTheaterInfoList:
                self.movieInTheaterInfoList.append(movieInfo)
        

    def closed(self, reason):
        with open('MovieInfosInTheater.json', 'w+') as f:
            f.write(json.dumps(self.movieInTheaterInfoList, indent=4, separators=(',', ':')))
